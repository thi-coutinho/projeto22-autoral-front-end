...
import { IElement } from "@/app/dashboard/page";
...
export default function ToolBox({ element, setElement }: props) {
  const Icons: {
  ...
  return (
    <div className="bg-white w-fit h-fit flex items-center text-black p-2 rounded-lg gap-3 shadow-md shadow-black absolute top-2 inset-x-0 mx-auto">
      {Icons.map((obj, i) => (
        <Tooltip key={i} title={`Create ${obj.elementText}`}>
          <obj.component
            onClick={(e) => {
              e.stopPropagation();
              setElement(obj.elementText);
            }}
            className="rounded hover:bg-slate-200 p-1"
            sx={{
              outline: `${
